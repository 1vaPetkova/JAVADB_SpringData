...
//        String alterQuery = String.format("ALTER TABLE %s ADD COLUMN %s;", "`users`",
//                getNewFields(entity.getClass()));
//        return connection.prepareStatement(alterQuery).execute();
//    }
//
//    private <T> String getNewFields(Class<?> entityClass) throws SQLException {
//        StringBuilder sb = new StringBuilder();
//        Set<String> fields = getAllColumnsInTable(entityClass);
//        List<Field> collect = Arrays.stream(entityClass.getDeclaredFields())
//                .filter(f -> f.isAnnotationPresent(Column.class)).collect(Collectors.toList());
//        Arrays.stream(entityClass.getDeclaredFields())
//                .filter(f -> f.isAnnotationPresent(Column.class))
//                .forEach(f -> {
//                    String fieldName = f.getName();
//                    if (!fields.contains(fieldName)) {
//                        sb.append(getNameAndTypeOfField(f));
//                    }
//                });
//        return sb.toString();
//    }
...
//    private <T> Set<String> getAllColumnsInTable(T entity) throws SQLException {
//        Set<String> allColumns = new HashSet<>();
//        String query = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS\n" +
//                "WHERE TABLE_SCHEMA = 'test_orm' AND COLUMN_NAME != 'id' AND `TABLE_NAME` = 'users';";
//        PreparedStatement preparedStatement = connection.prepareStatement(query);
//        ResultSet resultSet = preparedStatement.executeQuery(query);
//
//        while (resultSet.next()) {
//            allColumns.add(resultSet.getString("COLUMN_NAME"));
//        }
//        return allColumns;
//    }
